...
void PriorityQueue::Generate(PT pt)
{
    ...
    if (pt.content.size() == 1)
    {
        ...
        // guesses.reserve(guesses.size() + total_values);
        int chunk_size;
        if (total_values < 1000) {
            chunk_size = 20;  // 小任务用小块
        } else if (total_values < 10000) {
            chunk_size = 100; // 中等任务
        } else if (total_values < 100000) {
            chunk_size = 500; // 大任务
        } else {
            chunk_size = 1000; // 超大任务
        }
...
// 预先分配空间
        ...
        #pragma omp parallel num_threads(8)
        {
            // 每个线程使用本地向量存储猜测结果
            vector<string> local_guesses;
            // 为每个线程预分配适当的空间，减少扩容开销
            // local_guesses.reserve(total_values / omp_get_num_threads() + 100);
