...
void MD5Hash_simd(string input, bit32 *state) {
    ...
    uint32x4_t state_simd = vld1q_u32(state); // 加载初始 state 到 SIMD 寄存器

    // 逐 block 更新 state
    for (int i = 0; i < n_blocks; i++) {
        uint32x4_t a, b, c, d;
        uint32x4_t x[4]; // 每次处理 4 个 32 位数

        // 加载当前 block 的数据到 x
        for (int j = 0; j < 4; ++j) {
            x[j] = vld1q_u32(reinterpret_cast<const uint32_t *>(&paddedMessage[i * 64 + j * 16]));
        }

        a = vdupq_n_u32(state[0]);
        b = vdupq_n_u32(state[1]);
        c = vdupq_n_u32(state[2]);
        d = vdupq_n_u32(state[3]);
        ...
        GG_SIMD(d, a, b, c, x[2], s22, 0xc040b340);
        GG_SIMD(c, d, a, b, x[3], s23, 0x265e5a51);
        GG_SIMD(b, c, d, a, x[0], s24, 0xe9b6c7aa);

        // Round 3
        HH_SIMD(a, b, c, d, x[2], s31, 0xfffa3942);
        HH_SIMD(d, a, b, c, x[3], s32, 0x8771f681);
        HH_SIMD(c, d, a, b, x[0], s33, 0x6d9d6122);
        HH_SIMD(b, c, d, a, x[1], s34, 0xfde5380c);

        // Round 4
        II_SIMD(a, b, c, d, x[3], s41, 0xf4292244);
        II_SIMD(d, a, b, c, x[0], s42, 0x432aff97);
        II_SIMD(c, d, a, b, x[1], s43, 0xab9423a7);
        II_SIMD(b, c, d, a, x[2], s44, 0xfc93a039);

        // 更新 state
        state_simd = vaddq_u32(state_simd, vcombine_u32(vget_low_u32(a), vget_high_u32(b)));
    }

    // 将 SIMD state 写回普通数组
    vst1q_u32(state, state_simd);
