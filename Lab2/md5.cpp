...
void MD5Hash_simd(string input, bit32 *state) {
    ...
    // SIMD 初始化
    __m128i a, b, c, d;
    __m128i init_state = _mm_set_epi32(state[3], state[2], state[1], state[0]);

    // 逐 block 更新 state
    for (int i = 0; i < n_blocks; i += 1) {
        __m128i x[4]; // 每次处理 4 个 32 位整数
        for (int i1 = 0; i1 < 4; ++i1) {
            x[i1] = _mm_set1_epi32(
                (paddedMessage[4 * i1 + i * 64 + 3] << 24) |
                (paddedMessage[4 * i1 + i * 64 + 2] << 16) |
                (paddedMessage[4 * i1 + i * 64 + 1] << 8) |
                (paddedMessage[4 * i1 + i * 64]));
        }
        ...
        II_SIMD(d, a, b, c, x[1], s42, 0x432aff97);
        II_SIMD(c, d, a, b, x[2], s43, 0xab9423a7);
        II_SIMD(b, c, d, a, x[3], s44, 0xfc93a039);

        // 更新 state
        state[0] += _mm_extract_epi32(a, 0);
        state[1] += _mm_extract_epi32(b, 0);
        state[2] += _mm_extract_epi32(c, 0);
        state[3] += _mm_extract_epi32(d, 0);
    }
